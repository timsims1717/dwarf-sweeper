...
var xrayItem = &player.Item{
	...
	Name:    "XRay",
	Temp:    true,
	OnUseFn: func(_, _ pixel.Vec, e *ecs.Entity, secs float64) bool {
		if t, ok := e.GetComponentData(myecs.Transform); ok {
			trans := t.(*transform.Transform)
			myecs.Manager.NewEntity().
				AddComponent(myecs.Transform, transform.New()).
				AddComponent(myecs.Temp, timing.New(secs)).
				AddComponent(myecs.Update, data.NewFrameFunc(func() bool {
					n := Descent.Cave.GetTile(trans.Pos).RCoords.Neighbors()
